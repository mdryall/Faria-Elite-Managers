...
\begin{abstract}
\noindent Prosperity, on an influential recent account, breeds elite overproduction: aspirants to elite positions multiply faster than the positions themselves, intensifying intra-elite competition and political instability.
We study a distinct, adjacent market mechanism: prosperity can also expand the filled elite sector and the resources absorbed by its institutions.
...
Competition does eliminate an incumbent group type dominated in the complete product it sells, status included: in an economy containing both types, a rival supplying equivalent standing at lower resource cost prevents the dominated incumbent from operating in any equilibrium.
...
Dynamically, richer societies support a larger filled elite sector, and our recursive example converges to lower steady-state wealth than its status-free counterfactual.
\medskip
...
In \emph{End Times}, the historian Peter Turchin argues that societies at a certain stage of development begin to overproduce \emph{elite aspirants}: the number seeking elite positions grows faster than the positions available, intensifying intra-elite competition and political instability \citep{turchin2023endtimes}.
...
Our model does not formalize Turchin's surplus of aspirants.
It studies an adjacent market mechanism --- the endogenous expansion of \emph{filled} elite positions and the institutions supporting them --- and asks when that expansion absorbs resources without being competed away.
...
Part of the reason may lie in what these institutions actually sell.
Elite institutions absorb substantial resources per graduate, and how much of what they sell is skill remains contested.
Selection-adjusted comparisons find modest average effects of attending a more selective college on log earnings after conditioning on students' application and admission sets, with larger estimated returns for disadvantaged students \citep{dalekrueger2002}.
A regression-discontinuity design at elite Chilean business programs finds large effects on leadership and top incomes concentrated among men from high-tuition private high schools \citep{zimmerman2019elite}.
An admissions-based design for Ivy-Plus colleges finds large effects on top-one-percent earnings, elite graduate-school attendance, and employment at prestigious firms \citep{chettydemingfriedman2026}.
Among interwar Harvard students, membership in the relevant social clubs is descriptively associated with a larger labor-market premium than academic performance \citep{michelmanpricezimmerman2022}, while elite professional-service firms screen heavily on pedigree and polish \citep{rivera2015pedigree}.
These estimates concern private effects for admitted students; they do not by themselves identify the aggregate material return to expanding the elite sector.
...
Competition does eliminate dominated incumbent group types, in the following exact sense: when an economy contains both an incumbent and a feasible \emph{membership-equivalent} replacement --- one whose seats carry the same qualifications and the same utility for their holders --- the incumbent cannot be active in any equilibrium if the replacement uses strictly fewer resources.
...
Elite managers earn a gross wage premium over their competently trained peers, pinned by their firms' revenue; net of education costs, however, they accept a strictly lower pecuniary position --- the status discount that finances the system (Corollary~\ref{cor:wages}).
...
In the baseline, the incidence is the reverse of Turchin's wealth-pump mechanism: the elite do not extract the system's resource cost from the rest of society; they finance it through the pecuniary status discount they accept.
The same accounting yields a sharp characterization: in an interior equilibrium, \emph{strictly less productive} elite firms operate exactly when the status discount exceeds the workers' compensating differential plus the resources burned (Proposition~\ref{prop:coexist}) --- though in that polar case the gross premium necessarily reverses, so the empirically relevant configuration is the one in which elite firms are more productive per firm and their managers visibly better paid.
...
Status is a luxury in equilibrium: because the elite sector's waste is a fixed resource cost while wages scale with wealth, richer economies support a larger filled elite sector (Proposition~\ref{prop:luxury}).
This price-mediated expansion is distinct from, but complementary to, Turchin's mechanism based on excess aspirants competing for a limited stock of positions.
And the elite sector drags on accumulation: rising wealth expands the status spending that absorbs it, and the steady state settles permanently below the status-free counterfactual (Proposition~\ref{prop:drag}, under single-crossing provisos our example satisfies).
In the maintained near-origin regime, the elite sector is inactive and the origin repels when the return condition in Proposition~\ref{prop:map} holds; this is a conditional local result, not a global no-collapse theorem for unmodeled corner regimes.
The baseline law of motion is recursive because parents have warm-glow preferences rather than a forward-looking argument.
Within the degree-one common-factor class of Proposition~\ref{prop:invariance}, correct anticipation leaves the within-period status margin unchanged at a given wealth, although the savings rule can change both the transition path and its destination.

Three features define the paper's contribution, which Section~\ref{sec:lit} positions relative to prior work.
...
And the welfare analysis derives both boundaries the paper draws --- between what competition eliminates and what it sustains, and between efficient status consumption and a genuine rat race --- within one linked school--qualification--firm economy.
...
Relative to these papers, our contribution is to locate status in a two-stage sequence of school and firm memberships within a competitive club economy, so that qualification, institution-level resource use, incidence, core properties, and aggregate dynamics can be studied jointly.
...
The baseline does not derive why an entrant cannot confer a partially or fully portable elite qualification.
...
That is a statement about the maintained portability and credibility of qualifications; the baseline does not explain how those properties arise or erode.
...
\section{Qualification Portability and Recognition}\label{sec:portability}
...
\subsection{Continuation of the numerical example}

Continue the parameters of Section~\ref{sec:example} and set $c_R=9/4$.
Then
\[
  D=\frac34,
  \qquad
...
At $\varrho_A$ the portable route enters at the baseline cutoff
$\eta_E=9/23$.
...
Thus qualification portability eliminates the incumbent pedigree without necessarily
eliminating --- and here while expanding --- the material drag associated with demand for
elite organization.
...
four-type group set $G$; incorporating partially recognized managers into the positional
dilution aggregate or the transition map requires a separate integration decision.
...
Competition remains exacting about the products markets actually trade: a membership-equivalent group technology with lower resource cost would displace the dominated group type in every equilibrium of the augmented economy (Theorem~\ref{thm:displacement}); what survives in the baseline is dominated only in the material projection of its product.
This paper has embedded priced elite status in a competitive general-equilibrium model of groups and traced the consequences through displacement, compensation, output, welfare, and accumulation.
...
\subsection{The compensated utilitarian cutoff family, the wedge \eqref{eq:wedge}, and Proposition~\ref{prop:tax}}
