% TRUTH VERSION: elite_status-v02.tex
% REVISION CANDIDATE: elite_status-v03.tex
% FILE ROLE: STAGE 3 STATIC-PORTABILITY INTEGRATION CANDIDATE; elite_status-v02.tex REMAINS AUTHORITATIVE
...
Elite institutions can persist even when their expansion lowers aggregate output: managers finance the resource cost by accepting lower net pecuniary compensation, while visible gross salary premia can coexist because salaries partly reimburse education costs.
With absolute status, the resulting equilibrium lies in the strong core and is Pareto optimal.
Competition eliminates an incumbent only when a replacement supplies equivalent membership and qualification value at lower resource cost.
Relaxing the baseline qualification boundary produces two recognition thresholds: a portable route first enters alongside the incumbent and then displaces it once recognition is sufficiently high.
Displacement need not raise output because the cheaper route also expands demand for elite-format management.
With positional status, entry dilutes incumbents, the welfare result fails, and a membership tax can improve welfare in our example.
In an overlapping-generations extension, status becomes more affordable as wealth rises and, under the resource-productivity and single-crossing conditions, lowers steady-state wealth relative to the status-free counterfactual.
\medskip
...
same product.
Yet elite schools and the firms their graduates run persist --- and often expand --- amid
competition in education, labor, and product markets.
The apparent contradiction rests on what counts as the product.
A material comparison records inputs and output; the market trades a complete product
that can also include membership, qualification, and standing.
This paper asks when that difference allows a resource-intensive elite sector to survive,
when competition displaces it, and when its survival constitutes a market failure.
...
Agents differ only in their taste for elite-manager status.
There is no hidden information, market power, admission rationing, or exogenous capacity
constraint.

The baseline equilibrium separates the salary an elite manager receives from the net
pecuniary position delivered by the complete education--career chain.
Elite managers can earn visible gross salary premia while accepting lower net income after
education costs; the difference is the status discount that finances the elite complex
(Corollary~\ref{cor:wages}).
As status tastes spread, the elite sector expands and aggregate output falls exactly when
that sector produces less output per unit of resources than the competent sector
...
When status is an absolute amenity and its full resource cost is priced, lower material
output is the cost of consumption, not by itself a market failure.

Competition remains exacting about complete products.
Theorem~\ref{thm:displacement} shows that an incumbent group type cannot operate when the
economy also contains a strictly cheaper replacement whose memberships confer the same
qualifications and utility.
Material dominance alone is insufficient.
In the baseline, the serious school uses fewer resources than the elite school but does not
confer the qualification required for elite management; it therefore fails membership
equivalence at precisely the point on which displacement turns.

Section~\ref{sec:portability} relaxes that binary qualification boundary.
A serious-school graduate can manage an elite-format firm after a real conversion or
verification input, but the resulting route receives only a fraction $\varrho$ of the
incumbent route's status recognition.
Two thresholds organize the market (Proposition~\ref{prop:portability}).
Below the activation threshold, the portable route is technologically feasible but not
chosen.
At intermediate recognition, nonelite, portable, and incumbent careers coexist, with
status taste sorting agents across them.
At and above the distribution-free displacement threshold, the portable route strictly
dominates the incumbent route; at full recognition no equilibrium activates the incumbent
school--firm chain (Corollary~\ref{cor:chain-displacement}).
This displacement result does not imply that output must rise.
Recognition both substitutes a cheaper chain for the incumbent and expands total demand
for elite-format management, so aggregate resource use and output are generally
ambiguous.

The baseline welfare result identifies rather than dismisses the source of a possible
market failure.
...
Thus the policy boundary is not waste versus no waste.
It is absolute status, whose cost is priced, versus positional status, whose dilution
effect is not.

The overlapping-generations extension in Section~\ref{sec:dynamics} returns to the
original four-type technology so that accumulation can be studied without adding further
portability assumptions.
The static economy becomes the period economy: each generation inherits resources, forms
schools and firms, and leaves a warm-glow bequest.
Under the same resource-productivity condition that governs the static output decline,
status is a luxury --- its fixed resource cost becomes easier to finance as wealth rises
--- and the elite sector lowers the transition map wherever it is active
(Propositions~\ref{prop:luxury} and~\ref{prop:drag}).
Under the single-crossing conditions satisfied by the example, steady-state wealth is
therefore below the status-free counterfactual.
The law of motion is recursive because preferences contain no forward-looking argument;
the paper does not claim to solve a forward-looking dynastic problem or to rank
...
The contribution is therefore a set of connected boundaries within a single economy
linking schools, qualifications, and firms: between material and complete-product competition,
between qualification feasibility and recognized portability, and between efficient
status consumption and a positional rat race.
Section~\ref{sec:lit} relates these results to work on status, education, organizations,
and growth.
Sections~\ref{sec:framework}--\ref{sec:model} define the framework and economy.
Section~\ref{sec:equilibrium} characterizes the baseline and proves the financing,
output, welfare, and displacement results.
Section~\ref{sec:portability} develops qualification portability, and
Section~\ref{sec:example} presents the numerical examples.
Sections~\ref{sec:positional} and~\ref{sec:dynamics} study positional status and
accumulation using the original group set.
Sections~\ref{sec:empirics}--\ref{sec:conclusion} develop the empirical interpretation,
discussion, and conclusion.
Appendices collect the proofs and companion-code references.

\section{Related Literature}\label{sec:lit}

\textbf{Markets for status.}
The idea that status itself can be traded has an established lineage.
The theory of equalizing differences treats job amenities as attributes with implicit competitive prices \citep{rosen1974hedonic,rosen1986equalizing}; the status discount below applies that established logic rather than claiming that nonpecuniary job attributes are newly priced.
\citet{lavetti2023compensating} surveys the empirical difficulties in recovering such differentials from observed wages and job choices.
...
Closest to our compensation mechanism, \citet{ferreiranikolowa2024} study firms that design careers to attract qualified workers to prestigious positions, jointly determining prestige, promotion, pay, employment, competition, and sector size.
Relative to these papers, our contribution is to locate status in a two-stage sequence of school and firm memberships within a competitive club economy and, in the static economy, to vary recognition of a portable qualification.
This permits analysis of institutional entry, displacement, resource incidence, and core properties, while the original group set supports the aggregate dynamics.
...
Here, by contrast, the supply of status is embedded in competitive general equilibrium, which allows welfare statements about the whole economy rather than about a designer's profit.
...
Applied work includes \citet{caucutt2001peer,caucutt2002vouchers}, who compute club equilibria of school economies with peer effects, and the firms-as-clubs analyses of \citet{prescotttownsend2006firms} and \citet{zame2007incentives}, the latter establishing that with strategic play inside firms, equilibria may be Pareto ranked.
A closely related group-market analysis is \citet{staab2024groups}, who studies endogenous group formation when agents value both peer quality and status among their peers, with uniformly priced membership, sorting, exclusion, market structure, and welfare as central objects.
Relative to this literature, the present model combines a two-stage qualification chain with production and follows the resulting club equilibria into a positional extension and an overlapping-generations transition.
The computed school economies of \citet{caucutt2001peer,caucutt2002vouchers} provide the closest methodological precedents for the period equilibrium; \citet{ferreiranikolowa2024} and \citet{staab2024groups} are closest on priced prestige and endogenous group or career formation.
...
A competitive market can sustain an output-reducing elite sector when status is part of
the product being sold.
In the baseline, elite managers finance the sector by accepting a lower net pecuniary
position, even when their gross salaries and firms' measured performance look superior.
Because the membership value and its resource cost are both priced, the resulting
absolute-status equilibrium lies in the strong core and is Pareto optimal.
Material waste alone is therefore not evidence of a market failure.

Competition still eliminates dominated complete products.
An alternative technology that is membership equivalent and uses fewer resources
prevents the incumbent group type from operating.
When the serious qualification becomes functionally portable but only partially
recognized, entry and displacement occur at distinct thresholds.
Sufficient recognition eliminates the incumbent pedigree, but need not improve aggregate
material performance: the cheaper route can expand total demand for elite-format
management faster than it reduces the resource cost of each chain.

The welfare conclusion changes when status is positional.
Entry then dilutes incumbent status, the equilibrium is neither Pareto optimal nor in the
core, and the numerical example admits a membership tax that raises welfare and output.
The dynamic extension, kept on the original four-type technology, adds a different
comparative static: under the resource-productivity condition, rising wealth expands
elite demand and the active elite sector lowers accumulation relative to the status-free
counterfactual.

The empirical task is consequently to distinguish four objects that standard firm or
education accounts combine: productive skill, membership value, qualification
portability, and recognition.
Those distinctions determine what competition can displace, who finances the surviving
institutions, and whether a policy problem exists.
