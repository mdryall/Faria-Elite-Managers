...
The premium decomposes exactly --- gross premium equals excess tuition minus the status discount --- so the status-financed component of elite pay is tuition reimbursement, not productivity, and the decomposition yields the model's sharpest prediction: changes in elite tuition have zero incidence on elite salaries, being absorbed by the status discount.
...
What the wage carries, by the accounting identity \eqref{eq:wagedecomp}, is tuition reimbursement net of the status discount --- in the limit of a vanishing discount ($\eta^\ast\to 0$), \emph{exactly} tuition reimbursement --- and what the identity prices that data do not show is the net position: subtract the cost of the elite education, and the elite career pays strictly less than the competent one.
...
(ii) The equilibrium exhibits over-entry: the laissez-faire cutoff sets the marginal entrant's surplus to zero, whereas every Pareto-optimal cutoff allocation requires that surplus to equal the strictly positive wedge \eqref{eq:wedge}.
In the example, utilitarian welfare is strictly increasing in $t$ at $t=0$ and the welfare-maximizing tax is interior.
...
Uniform inheritance abstracts from within-generation heterogeneity in inheritances --- the gift is pooled, so there are no dynasties in the model, only generations --- and the Galor--Zeira-style interaction of inherited inequality with elite membership is an important extension we defer.
...
We are careful about what this does and does not establish: with warm-glow preferences the absence of an expectational channel is a feature of the specification, and the natural sharpening --- parents who value their children's prospects directly --- is discussed in Section~\ref{sec:discussion}.
...
Under condition \eqref{eq:decline}, therefore: (i) the transition map lies strictly below the no-elite-sector map wherever the elite is active, so every steady state in that region is strictly poorer than its no-elite counterpart, $k^\ast_{\mathrm{elite}} < k^\ast_{\mathrm{none}}$ for the corresponding crossings (unique in our example); (ii) a FOSD spread of the status-taste distribution, strict at the relevant cutoffs, lowers $\xi$ pointwise (strictly where the elite is active) and strictly lowers each such steady state; (iii) under positional status, and for taste shifts satisfying the damping hypothesis of Proposition~\ref{prop:positional}(ii), both effects are strictly damped: $k^\ast_{\mathrm{abs}} < k^\ast_{\mathrm{pos}} < k^\ast_{\mathrm{none}}$ in the example.
\end{proposition}
