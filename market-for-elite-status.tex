...
If status is valued for its scarcity, an unpriced dilution externality generates over-entry, and a corrective membership tax can raise welfare and output together, as it does in our example.
...
First, equilibrium compensation reproduces the gross configuration compensation data show and pairs it with a net prediction those data do not yet test.
...
Bars also mark two bounds ($\bar\ell$, $\bar\eta$) alongside the aggregates, the count $N$ and utilitarian welfare $W$ are capitals by long convention, and the letter $m$ names both generic memberships (always paired with a group type or subscripted) and the elite mass of Section~\ref{sec:positional}.
...
(iii) At $t=0^{+}$, the output response decomposes into a de-entry term and a rebate term, $d\bar{y}/dt = \bar{y}_m\,m'(0^{+}) + \bar{y}_T\,m^\ast$, where $\bar{y}_m$ is the derivative of Proposition~\ref{prop:decline} (positive times $m'<0$ under condition \eqref{eq:decline}) and $\bar{y}_T<0$ is the rebate's effect through reservation incomes and the output price; output rises if and only if the de-entry term dominates.
\end{proposition}
...
Under positional status, whenever entry responds strongly enough that the de-entry term dominates the rebate's price effect, the tax raises output and welfare simultaneously, so there is no equity--efficiency trade-off: the taxed activity was destroying value at the margin in both senses.
...
In the example the de-entry term is $+0.016$ against a rebate term of $-0.006$, so both margins improve (Appendix~\ref{app:positional}).
...
Against the presumption that elite waste is a market failure demanding correction, the model shows the waste is Pareto optimal --- unless status is valued for its scarcity, in which case, and only in which case, there is over-entry, which a corrective membership tax can convert into gains in output and welfare together, as in our example.
...
(iii) At $t=0^{+}$, $d\bar{y}/dt = \bar{y}_m m'(0^{+}) + \bar{y}_T m^\ast$ with $\bar{y}_m = \phi[\alpha(c+r)-\beta c-\tau r]/[2c+2r+(\mu+2\lambda-3)k]$, the identity of Proposition~\ref{prop:decline} evaluated at reservation income $k+t\,m$, and $\bar{y}_T<0$ obtained by differentiating market clearing in the rebate.
Under \eqref{eq:decline} the first term is positive when $m'<0$; it need not dominate --- a taste distribution putting little mass at the cutoff makes $|m'(0^{+})|$ small while $m^\ast$ remains bounded away from zero, and output then falls with the tax --- which is why (iii) states the dominance condition rather than an unconditional sign.
In the example the terms are $+0.016$ and $-0.006$ (computed in the companion script), so output rises.
